...
This renders every template in `templates/` into `configs/wm-configs/` (window manager, Kitty, Polybar, Rofi) and `configs/tool-configs/` (vim, vifm, and the generated vifm colorscheme), then copies each to its system location.

Warning: generation replaces Kitty's active config at `~/.config/kitty/kitty.conf`, herbstluftwm's active autostart at `~/.config/herbstluftwm/autostart`, `~/.vimrc`, vifm config at both `~/.vifm/vifmrc` and `~/.config/vifm/vifmrc`, and the generated vifm colorscheme at both `~/.vifm/colors/atta-wm.vifm` and `~/.config/vifm/colors/atta-wm.vifm`. Back up your existing files before running the generator if you want to keep your current setup.
